...
3. Choose the newly created Aspect type `Data Tier` and click <walkthrough-spotlight-pointer locator="semantic({button 'Edit'})"></walkthrough-spotlight-pointer>. Next, add a field (under *Template*, click <walkthrough-spotlight-pointer locator="semantic({button 'Add Field'})">Add Field</walkthrough-spotlight-pointer>):
