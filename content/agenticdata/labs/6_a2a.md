...
The analyst's full specification — its one tool, the Conversational Analytics wiring, the A2A server — lives in <walkthrough-editor-open-file filePath="content/agenticdata/src/adk/AGENTS.md">AGENTS.md</walkthrough-editor-open-file> in the project folder. Read it first, then start ``agy`` (you are already in ``content/agenticdata/src/adk``) and paste:
...
5. **Watch the engine room**: every concierge turn and every A2A delegation you just ran was a Gemini call. Open the [Vertex AI dashboard](https://console.cloud.google.com/vertex-ai) and find **Model observability** — requests, tokens, and latency per model; **Show all metrics** opens the full dashboard in Cloud Monitoring.
